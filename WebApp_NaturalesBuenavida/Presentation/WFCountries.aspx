<%@ Page Title="Gestión de Unidades de Medida" Language="C#" MasterPageFile="~/Main.Master" AutoEventWireup="true" CodeBehind="WFCountries.aspx.cs" Inherits="Presentation.WFCountries" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="container mt-4 bg-white border rounded">
        <table id="countriesTable" class="table display" style="width: 100%">
            <thead>
                <tr>
                    <th>ID</th>
                    <th>Codigo</th>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#countriesTable').on('click', '.edit-btn', function () {
                const rowData = $('#countriesTable').DataTable().row($(this).parents('tr')).data();
                loadData(rowData);
            });

            // Manejo del clic en el botón de "eliminar" de la tabla
            $('#countriesTable').on('click', '.delete-btn', function () {
                const countryID = $(this).data('id');       // Obtengo el ID de la unidad a eliminar
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.success) {
                                ...
                                alert('Error al eliminar el pais: ' + response.Message);
                            }
                        },
                        error: function () {
